...
class DiscordRichPresenceApp:
    def __init__(self):
        self.presence = None
        self.running = False
        self.update_thread = None  # Adicionar um controle para a thread de atualização
    ...
    def stop_presence(self):
        ...
        if self.update_thread:
            self.update_thread.join()  # Espera a thread de atualização terminar
    ...
    def update_presence(self, state, details, large_image, small_image, start_time, show_state, show_details, show_large_image, show_small_image):
        if self.presence:
            ...
            # Reiniciar o loop de atualização com as novas configurações
            if self.update_thread:
                self.update_thread.join()  # Espera a thread de atualização terminar
            self.update_thread = threading.Thread(
                target=self.start_presence,
...
def main(page: ft.Page):
    ...
    client_id = txt_client_id.value  # Adicionar a linha para pegar o client_id da interface
    state = txt_state.value
    details = txt_details.value
    large_image = txt_large_image.value
    small_image = txt_small_image.value
    ...
    app.update_presence(client_id, state, details, large_image, small_image, start_time, show_state, show_details, show_large_image, show_small_image)
